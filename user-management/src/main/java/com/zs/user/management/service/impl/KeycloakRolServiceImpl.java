...
public class KeycloakRolServiceImpl implements IKeycloakRolService {
    ...
    public List<UserRepresentation> usersOfRole(String rolAdi) {
        List<UserRepresentation> list = keycloak.realm(realm).roles().get(rolAdi).getRoleUserMembers().stream().filter(val -> Objects.nonNull(val.getFirstName())).collect(Collectors.toList());
